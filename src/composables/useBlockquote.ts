...
export function useBlockquote(config?: UseBlockquoteConfig) {
  ...
  const canToggle = ref(canToggleBlockquote(editor.value))
  const isActive = ref(editor.value?.isActive('blockquote') || false)
  ...
  watch(
    ...
    (newEditor) => {
      ...
      const handleSelectionUpdate = () => {
        ...
        canToggle.value = canToggleBlockquote(newEditor)
        isActive.value = newEditor.isActive('blockquote')
      }

      handleSelectionUpdate()

      newEditor.on('selectionUpdate', handleSelectionUpdate)
      newEditor.on('transaction', handleSelectionUpdate)

      return () => {
        newEditor.off('selectionUpdate', handleSelectionUpdate)
        newEditor.off('transaction', handleSelectionUpdate)
      }
    },
    { immediate: true }
  )
