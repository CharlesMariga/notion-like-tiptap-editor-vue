...
export const listIcons: Record<ListType, Component> = {
  bulletList: markRaw(ListIcon),
  orderedList: markRaw(ListOrderedIcon),
  taskList: markRaw(ListTodoIcon),
}
...
/**
 * Custom composable that provides list functionality for Tiptap editor
 */
export function useList(config: UseListConfig) {
  ...
  const canToggle = computed(() => canToggleList(editor.value, type))
  const isActive = computed(() => isListActive(editor.value, type))
