...
import type { Level } from '@/composables/useHeading'
...
/**
 * Custom composable that provides turn into dropdown functionality for Tiptap editor
 *
 * @example
 * ```ts
 * // Simple usage
...
export function useTurnIntoDropdown(config?: UseTurnIntoDropdownConfig) {
  ...
  const activeBlockType = computed(() => getActiveBlockType(editor.value, blockTypes))
  ...
  // Watch for selection changes in editor
  watch(
    editor,
    (newEditor, oldEditor) => {
      if (oldEditor) {
        oldEditor.off('selectionUpdate', updateVisibility)
      }
  ...
  return {
    ...
    activeBlockType,
    handleOpenChange,
    filteredOptions: computed(() => getFilteredBlockTypeOptions(blockTypes)),
    label: computed(
      () => `Turn into (current: ${activeBlockType.value?.label || 'Text'})`
    ),
    Icon: markRaw(ChevronDownIcon),
  }
}
