...
export const headingIcons: Record<Level, Component> = {
  1: markRaw(HeadingOneIcon),
  2: markRaw(HeadingTwoIcon),
  3: markRaw(HeadingThreeIcon),
  4: markRaw(HeadingFourIcon),
  5: markRaw(HeadingFiveIcon),
  6: markRaw(HeadingSixIcon),
}
...
/**
 * Custom composable that provides heading functionality for Tiptap editor
 */
export function useHeading(config: UseHeadingConfig) {
  ...
  const canToggleState = computed(() => canToggle(editor.value, level))
  const isActive = computed(() => isHeadingActive(editor.value, level))
