...
/**
 * Custom composable that provides image download functionality for Tiptap editor
 */
export function useImageDownload(config?: UseImageDownloadConfig) {
  ...
  onMounted(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      // Check for mod+shift+d
      const isMod = event.metaKey || event.ctrlKey
      if (isMod && event.shiftKey && event.key.toLowerCase() === 'd') {
        if (!isVisible.value || !canDownload.value || isMobile) return
  ...
  return {
    ...
    Icon: markRaw(ArrowDownToLineIcon),
  }
}
